...
export class EditPostPage extends React.Component {
  ...
  createMediaElement = (description, media) =>
    this.context.dispatch(createMediaElement(description, media));

  invalidateMediaForm = (errors) =>
    this.context.dispatch(invalidateMediaForm(errors));

  deleteMediaElement = (uid) => this.context.dispatch(deleteMediaElement(uid));

  setMediaElementsCurrentPage = (page) =>
    this.context.dispatch(setMediaElementsCurrentPage(page));
  ...
  handleSubmit = (values) => {
    ...
    return this.context.dispatch(fn(values)).then(() => {
      if (this.context.state.postFormErrors.length === 0) {
  ...
  render() {
    ...
    const {
      ...
      mediaElements,
      mediaFormErrors,
      mediaElementsCurrentPage,
      mediaElementsNextPage,
    } = this.context.state;
    const defaultValues =
    ...
    const mediaLibraryProps = {
      ...
      createMediaElement: this.createMediaElement,
      invalidateMediaForm: this.invalidateMediaForm,
      deleteMediaElement: this.deleteMediaElement,
      setMediaElementsCurrentPage: this.setMediaElementsCurrentPage,
      currentPage: mediaElementsCurrentPage,
      nextPage: mediaElementsNextPage,
      mediaElements,
      mediaFormErrors,
    };
